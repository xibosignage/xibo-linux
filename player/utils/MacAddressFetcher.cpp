...
boost::optional<std::string> MacAddressFetcher::getMacAddress()
{
    try
    {
        auto socket = openSocket();
        auto interfaceConfig = getInterfaceConfig(socket);
    ...
    {
        Log::trace("MAC address was not fetched: {}", error.what());
        return {};
    }
}
...
ifconf MacAddressFetcher::getInterfaceConfig(SocketDescriptor socket)
{
...
std::string MacAddressFetcher::retrieveMacAddress(SocketDescriptor socket, ifreq& interfaceRequest)
{
    if(!ioctl(socket, SIOCGIFHWADDR, &interfaceRequest))
    {
        ...
        std::sprintf(macAddress, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
