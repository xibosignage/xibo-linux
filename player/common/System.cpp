...
DBusException::DBusException(DBusError error) :
    m_error(error)
{
}
...
{
    auto message = dbus_message_new_method_call("org.freedesktop.ScreenSaver", "/ScreenSaver", "org.freedesktop.ScreenSaver", "Inhibit");
    if(!message) throw DBusException{m_error};

    bool result = dbus_message_append_args(message, DBUS_TYPE_STRING, &pid, DBUS_TYPE_STRING, &reason, DBUS_TYPE_INVALID);
    if(!result)
    {
