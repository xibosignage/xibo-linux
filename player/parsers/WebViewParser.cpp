...
WebViewOptions WebViewParser::parse()
{
    ...
    auto transparency = node().get<bool>(ResourcesXlf::option(ResourcesXlf::Media::WebView::Transparent), DEFAULT_TRANSPARENCY);
    auto mode = node().get<int>(ResourcesXlf::option(ResourcesXlf::Media::WebView::ModeId), DEFAULT_WEBVIEW_MODE);

    return WebViewOptions{options, transparency, mode};
}
