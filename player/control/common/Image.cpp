...
Image::Image(int width, int height) : Widget(handler_)
{
    handler_.set(Gdk::Pixbuf::create(Gdk::COLORSPACE_RGB, false, 8, width, height));
}
...
{
    handler_.set(Gdk::Pixbuf::create_from_file(path, width(), height(), preserveAspectRatio));
}
