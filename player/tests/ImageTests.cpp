...
TEST(ImageTest, Duration_Default_DurationEqualsDefault)
{
    auto [image, imageHandlerStub] = constructImage();

    ASSERT_EQ(image->duration(), DEFAULT_DURATION);
}

TEST(ImageTest, SetDuration_NewValue_DurationEqualsNewValue)
{
    auto [image, imageHandlerStub] = constructImage();

    image->setDuration(DEFAULT_DURATION + 1);

    ASSERT_EQ(image->duration(), DEFAULT_DURATION + 1);
}

TEST(ImageTest, Start_WithAttachedMedia_AttachedMediaStartShouldBeCalled)
{
    auto [image, imageHandlerStub] = constructImage();
    auto mockMedia = createMedia();
    image->attachMedia(unique(mockMedia));
...
TEST(ImageTest, Stop_WithAttachedMedia_AttachedMediaStopShouldBeCalled)
{
    auto [image, imageHandlerStub] = constructImage();
    auto mockMedia = createMedia();
    image->attachMedia(unique(mockMedia));
